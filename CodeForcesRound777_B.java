...
public class CodeForcesRound777_B {
    ...
    private static void solve() throws IOException{
        ...
        boolean[][] boolCheck = new boolean[n][m];
        ...
        for(int i=0;i<n;i++){
            for(int j=0;j<m;j++){
                if(!boolCheck[i][j] && arr[i][j] == '1'){
                    max_i = Integer.MIN_VALUE;
                    max_j = Integer.MIN_VALUE;
                    min_i = Integer.MAX_VALUE;
                    min_j = Integer.MAX_VALUE;
                    count = 0;
                    solveIT(arr, n, m, i, j, boolCheck);
                    if(count != (max_i - min_i + 1) * (max_j - min_j + 1)){
                        pos = true;
                    }
                }
            }
        }
        out.println(pos?"NO" : "YES");
    }
    private static void solveIT(char[][] arr, int n, int m, int i, int j , boolean check[][]) {
        if( i >= n || i < 0 || j < 0 || j >= m || arr[i][j] != '1' || check[i][j]) {
            return;
        }
        ...
        count++;
        check[i][j] = true;
        solveIT( arr , n , m , i+1 ,j , check );
        solveIT( arr , n , m , i-1 ,j , check );
        solveIT( arr , n , m , i ,j+1 , check );
        solveIT( arr , n , m , i ,j-1 , check );
    }
