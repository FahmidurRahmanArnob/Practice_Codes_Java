...
class AprilCookOff_SticksAndRecs {
    ...
    private static void solve() throws IOException {
        ...
        for(int i=0;i<n;i++){
            int temp = in.nextInt();
            m.put(temp, m.getOrDefault(temp, 0) + 1);//putting the value in the map;
        }
        long x = 0;
        for(int i : m.keySet()){
            if(m.get(i) % 2 == 1){
                BigInteger bigOne = new BigInteger("2");
                BigInteger bigTwo = new BigInteger("3");
                BigInteger bigThree = bigOne.gcd(bigTwo);
                x++;
            }
        }
        for(;(n + x) % 4 != 0;) x++;
