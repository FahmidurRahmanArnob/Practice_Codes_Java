...
class CodeChef_ChefDiet {
    ...
    private static void solve() throws IOException{
        ...
        int supplyOfTheProtein , leftOverThatStoredForNextDay = 0, days = 0;
        for(int i=1;i<=n;i++){
            supplyOfTheProtein = arr[i];
            int demandOfProtein = k;
            leftOverThatStoredForNextDay += supplyOfTheProtein; // this basically stores the total protein that we have;
            if(demandOfProtein > leftOverThatStoredForNextDay){
                days = i;
                break;
            }
            leftOverThatStoredForNextDay = leftOverThatStoredForNextDay - k;
        }
        if(days == 0) System.out.println("YES");
