...
public class GameOnRanges {
    ...
    private static final TreeSet<Integer> empty = new TreeSet<>();
    private static TreeMap<Integer, TreeSet<Integer>> map;
    ...
    private static void solve() throws IOException{
        ...
        for(int i=0;i<n;i++){
            ...
            int finalI = i ;
            map.compute(nodes[i].l, (key, value)->{
                if(value == null) value = new TreeSet<>();
                value.add(nodes[finalI].r);
                return value;
            });
        }
        for(Node now : nodes){
            for(int d = now.l; d <= now.r; d++){
                if(checkRange(now.l, d - 1) && checkRange(d + 1, now.r)){
                    out.print(now.l);
                    out.print(' ');
                    out.print(now.r);
                    out.print(' ');
    ...
    private static class Node{int l, r;}
    private static boolean checkRange(int l, int r){
        if(l>r)return true;
        return map.getOrDefault(l, empty).contains(r);
    }
