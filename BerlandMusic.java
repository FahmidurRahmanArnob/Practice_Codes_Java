...
public class BerlandMusic {
    ...
    private static void solve() throws IOException{
        ...
        for(int i=0;i<n;i++)ratings[i] = in.nextInt();
        String likeDislike = in.next();
        ArrayList<Rating> highRatings = new ArrayList<>();
        ArrayList<Rating> lowRatings = new ArrayList<>();
        for(int i=0;i<n;i++){
            char rat = likeDislike.charAt(i);
            if(rat == '0'){
                lowRatings.add(new Rating(ratings[i], i));
            }
            else{
                highRatings.add(new Rating(ratings[i], i));
            }
        }
        Collections.sort(highRatings, (a, b)->b.rat - a.rat);
        Collections.sort(lowRatings, (a, b)->b.rat - a.rat);
        int maxRatings = n, curRatings = maxRatings;
        for(Rating r : highRatings){
            ratings[r.index] = curRatings--;
        }
        for(Rating r : lowRatings){
            ratings[r.index] = curRatings--;
        }
        for(int i : ratings) System.out.print(i + " ");
        System.out.println();
    }
    static class Rating{
        int rat, index;
