...
public class KalindromeArray {
    ...
    private static void solve() throws IOException{
        ...
        int iOne = 0, iTwo = n - 1;
        while(iOne < iTwo && arr[iOne]==arr[iTwo]){
            iOne++;
            iTwo--;
        }
        if(iOne >= iTwo){
            ...
            if(check(arr, iOne, iTwo, arr[iOne]) || check(arr, iOne, iTwo, arr[iTwo])){
                System.out.println("YES");
            }
            else{
    ...
    private static boolean check(int[] arr, int iOne, int iTwo, int toRemove){
        while(true){
            while(iOne<arr.length && arr[iOne] == toRemove)iOne++;
            while(iTwo>=0 && arr[iTwo] == toRemove) iTwo--;
            if(iTwo <= iOne) break;
            if(arr[iOne] != arr[iTwo])return false;
            iOne++;
            iTwo--;
        }
        return true;
