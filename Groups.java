...
public class Groups {
    ...
    private static void solve() throws IOException{
        ...
        for(long x = 0; x < 5; x++){
            for(long y = x + 1; y < 5; y++){
                long countX = 0, countY = 0;
                boolean everything = true;
                for(long i=0;i<n;i++){
                    if(arr[(int) i][(int) x] == 1){
                        countX++;
                    }
                    if(arr[(int) i][(int) y] == 1){
                        countY++;
                    }
                    if(arr[(int) i][(int) x] == 0 && arr[(int) i][(int) y] == 0){
                        everything = false;
                    }
                }
                if(countX >= n / 2 && countY >= n/2 && everything){
                    answerFound = true;
                }
            }
        }
        System.out.println(answerFound ? "YES" : "NO");
    }
