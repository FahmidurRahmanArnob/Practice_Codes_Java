...
public class CodeforcesRound778_C {
    @SuppressWarnings("FieldCanBeLocal")
    private static Reader in;
    private static PrintWriter out;
    private static int[] dp = new int[200006];
    private static int solveDp(char[] s, int i){
        if(i >= s.length) return 0;//array out bounds checker;
        if(dp[i] !=-1) return dp[i];
        if(i == s.length-1) return 1;//jei index e acho tar next index e same char;
        if(i < s.length - 1 && s[i] == s[i + 1]) return solveDp(s, i + 2);//
        else {
            int j;
            for(j=i + 1;j < s.length; j++){
                if(s[j] == s[i]) break;
            }
            if(j == s.length) return 1 + solveDp(s, i + 1);
            else{
                int one = j - i - 1 + solveDp(s, j + 1);
                int two = 1 + solveDp(s, i + 1);
