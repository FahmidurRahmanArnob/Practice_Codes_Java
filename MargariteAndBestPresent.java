...
public class MargariteAndBestPresent {
    ...
    private static void solve() throws IOException{
        ...
        int count = 0;
        if(n == m){
            count = (int) (Math.pow(-1, n % 2) * n);
        }
        else{
            int One = (int) (Math.pow(-1, n % 2) * n);
            int Two = (int) (Math.pow(-1, m % 2) * m);
            double Three = (One + Two) / 2.0;
            if(Three > 0){
                count = (int) Math.ceil(Three);
            }
            else{
                count = (int) Math.floor(Three);
            }
        }
