...
public class AquamoonAndTheStolenString {
    ...
    private static void solve() throws IOException{
        ...
        for(int i=0;i<m;i++){
            ...
            for(int j=0; j<n; j++){
                count[a[j].charAt(i) - 'a']++;//getting the ascii value of the char at ith and jth position; and storing it in arr[j];
            }
            for(int j=0; j<n-1; j++){
                count[b[j].charAt(i) - 'a']--;
            }
            for(int j=0; j<26; j++){
                if(count[j] != 0){
                    char total = (char) ( j + 'a' );
                    answerString.append(total);
                }
            }
        }
