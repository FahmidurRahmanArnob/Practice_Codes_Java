...
public class ChallengingCliffs {
    ...
    private static void solve() throws IOException{
        ...
        long[] mountains = new long[sizeOfArray];
        for(int i=0;i<sizeOfArray;i++)mountains[i] = in.nextLong();
        longRuffleSort(mountains);
        int minIndex = (int) 2e9;
        int index = -1;
        for(int i=1;i<sizeOfArray;i++){
            if(Math.abs(mountains[i] - mountains[i - 1]) < minIndex){//we are getting the first and the last mountains so that we can form uphill and downhill;
                minIndex = (int) Math.abs(mountains[i] - mountains[i - 1]);
                index = i;//these are the left over elements;
            }
            //as we studied here we are finding smallest abs difference ;
        }
        Vector<Integer> small = new Vector<>();//to store the small values;
        Vector<Integer> big = new Vector<>();// to store the big values;
        for(int i=0;i<sizeOfArray;i++){
            if(i == index || i == index - 1) continue;//if we find out the elements which are already taken out then we continue searching for others;
            if(mountains[i] > mountains[index - 1]) big.add((int) mountains[i]);//we store them here which are bigger to form a uphill; we are putting here those elements which are bigger than the smallest elements;
        }
        for(int i=0;i<sizeOfArray;i++){
            if(i == index || i == index - 1)continue;// doing the same checking for already taken out elements;
            if(mountains[i] <= mountains[index]) small.add((int) mountains[i]);//storing the smaller ones to form one(could be more but should be one) the downhiill(but it'll still be a uphill);// we are storing here those elements which are smaller than the biggest elements;
        }
        System.out.print(mountains[index - 1] + " ");
        ...
        System.out.print(mountains[index] + " " + '\n');
    }
