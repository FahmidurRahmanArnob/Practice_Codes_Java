...
public class Mex_Max_Cut {
    ...
    private static void solve() throws IOException{
        //Your Code Goes Here;
        int n = in.nextInt();
        String sOne = in.next(), sTwo = in.next();
        int ans = 0;
        for(int i=0;i<n;i++){
            if(sOne.charAt(i) != sTwo.charAt(i)) ans += 2;
            else{
                if(sOne.charAt(i) == '1'){
                    i++;
                    while(i < n && (sOne.charAt(i) != '0' && sTwo.charAt(i) != '0'))i++;
                    if(i<n)ans+=2;
                }
                else{
                    if(i + 1 == n) ans+=1;
                    else{
                        if(sOne.charAt(i+1) == '1' && sTwo.charAt(i+1) == '1'){
                            ans+=2;
                            i++;
                        }
                        else if(sOne.charAt(i+1) == '1' || sOne.charAt(i + 1) == '1'){
                            ans+=3;
                            i++;
                        }
                        else ans+=1;
