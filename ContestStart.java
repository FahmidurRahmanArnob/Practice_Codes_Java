...
public class ContestStart {
    ...
    private static void solve() throws IOException{
        //Your Code Goes Here;
        long one = in.nextInt(), two = in.nextInt(), three = in.nextInt();
        long a = three / two;
        long ans = 0;
        if(two > three) {System.out.println(0);}
        else if(two == three) {System.out.println(one - 1);}
        else if(a >= one) {ans = (one * (one - 1))/2;System.out.println(ans);}
        else {
            long z = one - a;
            ans += z * a;
            a -= 1;
