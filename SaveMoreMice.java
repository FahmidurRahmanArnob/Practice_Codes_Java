...
public class SaveMoreMice {
    ...
    private static void solve() throws IOException{
        ...
        long[] arr = new long[k];//Mices
        for(int i=0;i<k;i++)arr[i] = in.nextInt();
        longRuffleSort(arr);
        int count = 0;
        long sum = 0;
        for(int i=k - 1; i>=0; i--){//starting the loop from the hole to check the mice that is as near as the hole so that we can move that mice only;
            sum = sum + n - arr[i];//From i to k - 1 shobar pouchaite eituk time lagbe;
            if(sum < n){//comparing with the cats time if less then ++ ;
                count++;
            }
